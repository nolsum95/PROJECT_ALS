...
export default function EditUserModal({ open, onClose, user, onSuccess }) {
	...
	useEffect(() => {
		if (user) {
			setData({
				...
				password: '',
				password_confirmation: '',
				role_type: user.role_type ?? 'CAI',
			});
		}
	}, [user]);

	const updateRoute = (() => {
		try { return user ? route('users.update', user.user_id) : null; } catch { return null; }
	})();
	...
	return (
		<Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm" keepMounted key={user?.user_id ?? 'edit-user'}>
			...
			<form onSubmit={handleSubmit}>
				<DialogContent dividers>
					<Stack spacing={2}>
						...
						<TextField label="New Password (optional)" type="password" value={data.password} onChange={(e) => setData('password', e.target.value)} helperText={errors.password} error={!!errors.password} />
						<TextField label="Confirm New Password" type="password" value={data.password_confirmation} onChange={(e) => setData('password_confirmation', e.target.value)} helperText={errors.password_confirmation} error={!!errors.password_confirmation} />
